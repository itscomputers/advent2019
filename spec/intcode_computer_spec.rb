...
describe IntcodeComputer do
  let(:computer) { IntcodeComputer.new program: program, inputs: inputs, default_input: default_input }
  let(:inputs) { [] }
  let(:default_input) { nil }

  describe '#set and #get' do
    let(:program) { [5, 4, 3, 2, 1] }

    it 'can set position 2 to 1000' do
      expect(computer.result[2]).to_not eq 1000
      expect(computer.get 2).to_not eq 1000
      computer.set 2, 1000
      expect(computer.get 2).to eq 1000
      expect(computer.result[2]).to eq 1000
    end

    it 'can set position 500 to 1000' do
      expect(computer.result[500]).to be_nil
      expect(computer.get 500).to eq 0
      computer.set 500, 1000
      expect(computer.get 500).to eq 1000
      expect(computer.result[500]).to eq 1000
    end
  end
